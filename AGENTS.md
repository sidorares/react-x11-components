...
| MDX in `<Markdown>`                              | block position, here (`src/markdown/tags.ts` + `components` prop) | **M1 shipped — [docs/prd-mdx.md](docs/prd-mdx.md).** A `components` map gates the parser, so no document that rendered before can change; a tag is a component iff its name is a key in it. Attributes are strings/`true`/JSON — nothing is evaluated, which is the property that matters when the input is model output. Still open: `{expressions}` behind an opt-in `scope` (M2), and the inline half, which needs a `TextRun` that can reserve advance width for an embedded element — `<Html>`'s answer is a CSS line-box engine and is not borrowable. |
