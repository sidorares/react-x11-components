...
| MDX in `<Markdown>`                              | reserved seams only (`component` AST node)               | **Designed, not built — [docs/prd-mdx.md](docs/prd-mdx.md).** Composition-friendly by construction; the parser is the only part that grows. Streaming-compatible in principle. A `components` map gates the parser (so no existing document changes), attribute `{…}` is JSON until `scope` opts into `new Function`, and imports are a non-goal — this component's input is model output. |
