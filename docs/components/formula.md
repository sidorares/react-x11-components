...
| `size`           | `number`                 | Logical pixels per em — the unit a `fontSize` is written in; on a retina panel the element shapes at `size` × the display scale, like any style length. Default: theme `fontSize` × 1.21 — the ratio katex.css uses, so mathematics holds its own beside body text. |
