...
//
// **Vendored from `../tree/heights.ts`** — a component must not import
// another component, and the shared-module promotion is scheduled rather
// than done (docs/prd-table.md, M2). When it lands, delete both copies and
// import the shared one; until then a fix in one belongs in the other.
//
// The id-keying earns its place twice here: in the tree a measured height
// survives a collapse and re-expand; in a table it survives a **re-sort** —
// the permutation rebuilds offsets without remeasuring a single row.
//
...
// replaces for the rows a `render` seam made whatever height it liked.
//
...
import type { TableRowId } from './rows.js';
 ...
 *
 * Mutable and long-lived: one of these belongs to a `<Table>` for as long as
 * it is mounted, and `sync` re-points it at the rows a render is about to
 * draw. It is deliberately **not** React state — a measurement pass touches
 * it once per rendered row and the component re-renders once, on a version
 * counter, and only when something actually moved.
 */
export class RowHeights {
  /** Measured heights, by row id. Survives `sync`. */
  private readonly measured = new Map<TableRowId, number>();
   ...
   * rebuild it is O(n) — the rows change when items arrive, when a sort
   * flips, when a filter runs, all O(n) events already.
   */
   ...
   */
  measure(id: TableRowId, index: number, height: number): boolean {
