...
export class HtmlViewNode extends Node {
  ...
  private _prepare(width: number): void {
    ...
    if (this._stale === Stale.Nothing && this._laidOutFor === target) return;
    ...
    if (
      this._tree &&
      (this._laidOutFor !== target || this._stale >= Stale.Layout)
    ) {
